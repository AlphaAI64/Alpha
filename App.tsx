...
const App: React.FC = () => {
  ...
  /**
   * The 'Faster-Than-Iframe' Audit Trigger.
   * Directly redirects to the target URL. Native browser navigation is
   * significantly faster than injecting a 200KB+ iframe script and 
   * waiting for it to render a modal window.
   */
  const triggerAudit = () => {
    setIsAuditLoading(true);
    
    // Simulate top loading bar for immediate perceived speed
    ...
    // Native browser navigation is the fastest path. 
    // We use a small delay only to show the user the "Redirecting" state feedback.
    setTimeout(() => {
          ...
          <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
            ...
            <div className="hidden md:flex items-center gap-8 text-[12px] font-black uppercase tracking-widest text-slate-400">
              ...
              <Button 
                ...
                onMouseEnter={() => { const link = document.createElement('link'); link.rel = 'prefetch'; link.href = CALENDLY_URL; document.head.appendChild(link); }}
              >
                {isAuditLoading ? 'SECURE REDIRECT' : 'Book AI Audit'}
              </Button>
            </div>
          </div>
        </nav>
      ...
      <main>
          ...
          <div className="max-w-6xl mx-auto text-center">
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-6">
              <Button 
                ...
                onClick={triggerAudit} 
                loading={isAuditLoading} 
                className="rounded-full px-12 h-20 text-xl shadow-2xl"
              >
                {isAuditLoading ? 'ESTABLISHING CONNECTION' : 'Start Your AI Audit'}
              </Button>
            </div>
          </div>
        </section>
        ...
        <section id="services" className="py-40 px-6">
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-10 mb-24">
              ...
              <Button variant="outline" size="lg" onClick={triggerAudit} loading={isAuditLoading} className="rounded-full border-slate-200">
                {isAuditLoading ? 'REDIRECTING' : 'View Roadmap'}
              </Button>
            </div>
        ...
        <section className="py-60 px-6 text-center">
          <div className="max-w-4xl mx-auto">
            ...
            <Button size="lg" onClick={triggerAudit} loading={isAuditLoading} className="rounded-full h-24 px-16 text-2xl shadow-blue-500/20">
              {isAuditLoading ? 'INITIALIZING' : 'Book Your AI Audit'}
            </Button>
          </div>
        </section>
      </main>
      ...
      <footer className="py-24 px-6 border-t border-slate-100 bg-white">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-12">
          ...
          <div className="flex flex-col items-center md:items-end gap-4">
            <Button onClick={triggerAudit} variant="outline" size="sm" loading={isAuditLoading} className="rounded-full text-[10px]">Launch Audit</Button>
